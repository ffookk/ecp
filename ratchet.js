...
import { encodeBase64URL, decodeBase64URL, concatBytes, sha256, hkdfSHA256, hmacSHA256, encryptGCM, decryptGCM, encapsulateMLKEM1024, decapsulateMLKEM1024, keygenX25519, getSharedSecretX25519, keygenMLKEM1024, signComposite, verifyComposite, constantTimeCompare, randomUUID, } from './crypto.js';
...
const conversationId = (ek, kemCt) => encodeBase64URL(sha256(concatBytes(encodeUTF8('ECP-CONVERSATION-v2'), ek, kemCt)).slice(0, 16));
...
export const CreateInit = (contactFp) => withStateLock(async () => {
    ...
    const { contact, bytes: peerPub, publicIdentity: peer, } = await requirePeer(contactFp);
    ...
    const transcript = concatBytes(encodeUTF8('ECP-INIT-v2'), localPub, peerPub, ek.publicKey, kem.cipherText);
    ...
    let ciphertext;
    try {
        ciphertext = encryptGCM(symmetric.key, symmetric.nonce, zeros(0), concatBytes(header, fixed));
    }
    finally {
...
export const ProcessInit = (packet) => withStateLock(async () => {
    ...
    if (senderFp === calculateFingerprint(localPub))
        throw new Error('Self-messaging is prohibited.');
    const { contact, bytes: knownBytes, publicIdentity: sender, } = await requirePeer(senderFp);
    if (!constantTimeCompare(senderBytes, knownBytes))
        throw new Error('INIT sender is not the verified peer.');
    const transcript = concatBytes(encodeUTF8('ECP-INIT-v2'), senderBytes, receiverBytes, ek, kemCt);
    ...
    const replayId = encodeBase64URL(sha256(concatBytes(encodeUTF8('ECP-INIT-REPLAY-v2'), transcript)));
    ...
    try {
        ...
        let plaintext;
        try {
            plaintext = decryptGCM(symmetric.key, symmetric.nonce, packet.slice(offset), packet.slice(0, offset));
        }
        finally {
            clear(symmetric.key, symmetric.nonce);
        }
        ...
        const respHeader = buildHeader(Config.PACKET_TYPES.RESP, respPayload.length + 16);
        const responseKeys = deriveSymmetric(SK, 'ECP-RESP-v2', 'ECP-RESP-NONCE-v2');
        let respPacket;
        try {
            respPacket = concatBytes(respHeader, encryptGCM(responseKeys.key, responseKeys.nonce, respPayload, respHeader));
        }
        finally {
...
export const ProcessResp = (packet) => withStateLock(async () => {
    ...
    for (const session of sessions) {
        ...
        await requirePeer(session.contactFp, session.peerIdentity);
        const symmetric = deriveSymmetric(session.SK, 'ECP-RESP-v2', 'ECP-RESP-NONCE-v2');
        let plaintext;
        try {
            plaintext = decryptGCM(symmetric.key, symmetric.nonce, packet.slice(12), packet.slice(0, 12));
        }
        catch {
            continue;
        }
...
export const EncryptMessage = (contactFp, text) => withStateLock(async () => {
    ...
    try {
        ...
        const aad = concatBytes(encodeUTF8('ECP-MSG-v2'), cId, localPub, decodeBase64URL(session.peerIdentity), msgHeader);
        let ciphertext;
        try {
...
export const DecryptMessage = (packet) => withStateLock(async () => {
    ...
    const session = await DB.getByIndex('sessions', 'conversationId', encodeBase64URL(cId));
    if (!session || session.state !== 'ESTABLISHED' || !session.DHr)
        throw new Error('Established session not found for this message.');
    checkSession(session);
    await requirePeer(session.contactFp, session.peerIdentity);
    const aad = concatBytes(encodeUTF8('ECP-MSG-v2'), cId, decodeBase64URL(session.peerIdentity), serializeIdentityPublic(await getLocalIdentity()), packet.slice(12, offset));
    ...
    if (skipped[cacheKey]) {
        ...
        try {
            plaintext = decryptGCM(symmetric.key, symmetric.nonce, packet.slice(offset), aad);
        }
        finally {
        ...
        const derived = deriveMsgKeys(chain);
        try {
            plaintext = decryptGCM(derived.key, derived.nonce, packet.slice(offset), aad);
        }
        finally {
